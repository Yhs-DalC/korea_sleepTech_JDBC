...
* 
* 1) JDBC (JDBC API 기반)
*   : Jaba에서 DB에 접근할 수 있도록 표준으로 제공하는 API
*   - SQL문을 문자열로 작성
...
*   : SQL을 직접 작성하지 않고도, 자바 코드만으로 DB연동 가능
*   - Spring Boot에서는 주로 Hibernate를 JPA 구현체로 사용
* 
*   장점) SQL 작성이 거의 필요 X, '객체 중심의 개발 가능', 유지 보수가 쉽고 비즈니스 로직에 집중 가능
...
*   단점) SQL과 자바 코드가 분리 >> 유지 보수가 어렵고 중복 가능성 재, 코드량이 많음
* */
