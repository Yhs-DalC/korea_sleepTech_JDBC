...
// === SQL 실행 및 결과 처리 ===
// Statement VS PreparedStatement VS ResultSet1

// === SQL 실행 흐름 ===
// 1. Connection 객체 생성
// 2. SQL문 준비 (SELECT, INSERT 등)
// 3. Statement , PreparedStatement 객체 생성 - SQL 실행 객체 생성
// 4. SQL실행 (executeQuery, executeUpdate)
...
// 2) executeupdate
// : Insert, Update, Delete 와 같은 dml(data manipulation language)에서 실행 결과로 영향 받은 래코드 수를 반환
// - 반환 타입이 int
...
// 2) rs.getInt("컬럼명"), rs.getSting("컬럼명"), rs.getDate("컬럼명")
// : 정수값가져오기, 문자열값 가져오기, 날자값(DATE)가져오기
public class Jdbc03 {
}
