...
// 1. DAO패턴 정의
// : Data Access Object
// - DB와의 접근 로직을 하나의 클래스로 캡슐화하여 애플리케이션의 다른 부분과
// , DB 처리 코드를 분리하는 설계 패턴

// 2. DAO 패턴 사용 목적
// 1) DB 처리만 전담하는 DAO 클래스 생성
// 2) DAO 클래스만 수정하면 DB처리 수정이 가능
//      >> 구조 명확, 유지보수 용이

// 3. 구조 설계
// 1) DB - DBConnection.java (DB연결 전담)
// 2) entity - Member.java(데이터를 담는 객체)
// 3) dao - MemberDao.java (DB처리 로직 담당)
...
public class MemberDao {
    // 1. Read(단건): 아이디(id)를 기준으로 사용자 정보를 가져오는 메소드
    public Member getUserById(int id) throws SQLException{
        // +) get + 사용자 정보 + By + 찾고자 하는 컬럼명

        //DBConnection 을 통해 DB 연결을 가져옴
        Connection conn = DBConnection.getConnection();// static 메소드: 클래스명으로 호출

        // 실행할 SQL문 작성
        // : id를 조건으로 사용자 정보 조회(동적 파리미터)
        ...
        // 실행 객체에 파리미터 값 설정
        pstmt.setInt(1, id);// getUserById 메소드의 매개변수를 인자로 전달

        //SQL 쿼리 실행 >> 결과를 ResultSet 으로 저장
    ...
    // 2. Read(전체): 모든 사용자 정보를 조회하는 메소드
    public List<Member> findAll() throws SQLException{
        Connection conn = DBConnection.getConnection();

        //모든 사용자 정보를 조회하는 SQL쿼리문
    ...
    // 4. Update: 사용자 정보 수정 메소드(이름&이메일)
    public void updateMember(Member member) throws SQLException{
        Connection conn = DBConnection.getConnection();

        // 업데이트 할 이름과 이메일이 존재하는 지 확인
        ...
        // >> 메모리 낭비 + 속도 저하
        //String str = "hello";
        //str += " JDBC";// str에 JDBC를 추가한 것이 아니라 기존 문자열은 버려지고 새로운 String 객체가 생성
    ...
    public void deleteMember(int id) throws SQLException{
        // try_with_resources 구문(java7부터 도입)
        // : 자원을 자동으로 닫아주는 기능
        // try(자원 객체 선언){
        //      자원 사용
        //} catch (...){
